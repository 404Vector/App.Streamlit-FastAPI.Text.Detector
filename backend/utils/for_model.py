...
def detect(model, images, input_size, device):
    prep_fn = A.Compose([
    ...
    for image in images:
        ...
        img = prep_fn(image=image)['image']
        input = img.to(device=device)
        batch.append(input)
...
def load_model(device:str, ptr_path:str) -> models.EAST:
    model = models.EAST(pretrained=False).to(device)
    try:
        checkpoint = torch.load(ptr_path, map_location='cpu')
        model.load_state_dict(checkpoint['model_state_dict'])
    except:
        model.load_state_dict(torch.load(ptr_path, map_location='cpu'))
    model.eval()
    print("INFO:     model has been loaded.")
    return model

def inference(model:models.EAST, image:np.ndarray, input_size:int, device:str):
    result = detect(model=model, images=[image], input_size=input_size, device=device)[0]
    result = np.array(result, np.int32)
    cv2.drawContours(image=image, contours=result, contourIdx=-1, color=(255,0,0))
